...
public class HNF_CHL_F003 extends CrawlingHNFNew {

	public static void main(String[] args) {
		executeCommand(new HNF_CHL_F003(), args);
	}



	// todo 표준화가 가능하면 표준화할 것
	@Override
	protected boolean scrap(CrawlingProduct info) throws Exception {

		// (무)하나로 시작하는 건강보험 1종(표준형)
		...
		// STEP 2. 가입조건 비교
		logger.info("가입유형 설정");
		setPrevalenceType(info.textType);
	...
	private CrawlingTreaty getTreatyInfoFromTr(WebElement $tr) throws Exception {
		...
		//특약 보장금액 영역
		// WebElement $treatyAssureMoneyTd = $tdList.get(6);
