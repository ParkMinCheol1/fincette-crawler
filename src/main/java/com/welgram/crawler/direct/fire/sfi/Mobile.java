...
public class Mobile extends SeleniumCrawler implements Scrapable {

    public Mobile(WebDriver driver, WebDriverWait wait) {
        helper = new SeleniumCrawlingHelper(driver, wait);
    }

    public void closeAlarm() throws CommonCrawlerException {
        // 최저보험료로 가입하는 경우에는 ‘일부환급형’으로 진행됩니다. => 처리
        try {
            WaitUtil.loading(3);
            driver.findElement(
                By.xpath("//section[@id='V2Alert']//button[text()='확인']")).click();
            logger.info("환급형태 전환 알람 : 최저보험료로 가입하는 경우에는 ‘일부환급형’으로 진행됩니다");
        } catch (Exception e) {
            logger.info("환급형태 전환 알람 없음!");
        }

        try {
            WaitUtil.loading(1);
            driver.findElement(
                By.xpath("//section[@id='HealthDiscountGuide']//button[text()='확인']")).click();
            logger.info("건강 할인정보 알람: 실제 적용되는 할인보험료와 다를 수 있음");
        } catch (Exception e) {
            logger.info("건강 할인정보 알람 없음");
        }
    }
    ...
    protected void setSubPlan(CrawlingProduct info)
        ...
        List<WebElement> list = helper.waitPesenceOfAllElementsLocatedBy(
            By.cssSelector("#calc-dambolist-table label")
        );
    ...
    public void setBirthdayNew(Object obj) throws SetBirthdayException {
        ...
        try {
            helper.sendKeys3_check(By.cssSelector("input[id^='birth']"), info.fullBirth, "생년월일");
            WaitUtil.loading(1);
        } catch (Exception e) {
        ...
        try {
            ...
            helper.click(
                helper.waitVisibilityOfAllElementsLocatedBy(
                    By.cssSelector("ul[class*='autocomplete-list'] li button")).get(0)
            );
    ...
    protected List<CrawlingTreaty> getPageTreatyListFromMobileModal(CrawlingProduct info, Boolean customizedPlan) throws Exception {
        ...
        for (int i = 0; i < modalLiList.size(); i++) {
            ...
            if (joinStatusTxt.contains("미가입")) {
                ...
                if (info.mainYn.equals("Y") && (!customizedPlan || treatyInCustomizedPlan)) {
                    // 미가입된 상태입니다. 가입된 상태입니다.
...
//					WaitUtil.loading(1);
                ...
                if (customizedPlan && !treatyInCustomizedPlan) {
