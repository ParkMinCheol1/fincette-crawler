...
public class SFI_DSS_D019 extends CrawlingSFIDirect {

	public static void main(String[] args) {
		executeCommand(new SFI_DSS_D019(), args);
	}
	...
	protected boolean scrap(CrawlingProduct info) throws Exception {
		...
		setPrevalenceType(info.getTextType().split(",")[0].trim());
		modalCheck();
		...
		logger.info("플랜 설정");
		setPlan(info.getTextType().split(",")[1].trim());

		logger.info("특약 설정");
		// 원수사 오류로 잠시 주석처리. 차후 다시 풀어야함
		// 확인 버튼이 동작하지 않아 가입/미가입 처리는 되지 않으나 특약 비교차 사용
		setTreaties(info.getTreatyList());

//		logger.info("한번에 보장 변경 팝업을 닫기 위해 확인 버튼 클릭");
//		helper.doClick(By.id("btn-confirm"));
//		modalCheck();
	...
	public void crawlPremium(Object... obj) throws PremiumCrawlerException {
		...
		try {
			...
			WebElement $premiumAreaDiv = driver.findElement(By.id("coverage-premium"));
			WebElement $premiumStrong = $premiumAreaDiv.findElement(By.xpath(".//strong[@class='blind']"));
	...
	public void setTreaties(List<CrawlingTreaty> welgramTreatyList) throws SetTreatyException {
		...
		try {
			...
			for (String treatyName : dismatchedTreatyNameList) {
				...
				WebElement $treatyNameTd = $treatyAreaTbody.findElement(By.xpath(".//td[1][contains(text(), '" + treatyName + "')]"));
			...
			for (String treatyName : matchedTreatyNameList) {
				...
				helper.moveToElementByJavascriptExecutor(driver.findElement(By.xpath("//*[@id='coverage-list']")));
			...
			logger.info("팝업 닫기 버튼 클릭");
			click(By.id("btn-popup-x"));
			modalCheck();
