...
public class HWF_DSS_F132 extends CrawlingHWFAnnounce {

    // 한화 굿밸런스종합보험(연만기 갱신형)2310 무배당 1종(납입면제 고급형)
    public static void main(String[] args) {
        executeCommand(new HWF_DSS_F132(), args);
    }
    ...
    private void crawlFromAnnounce(CrawlingProduct info) throws Exception {
        ...
        logger.info("납입보험료 입력 후 다시 계산(해약환급금 조회 위해)");
        String monthlyPremium = info.treatyList.get(0).monthlyPremium;
        WebElement $premiumInputBox = driver.findElement(By.id("smPrm"));
        helper.sendKeys4_check($premiumInputBox, monthlyPremium);
