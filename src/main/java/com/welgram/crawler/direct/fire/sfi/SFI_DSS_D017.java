...
public class SFI_DSS_D017 extends CrawlingSFIDirect {

	public static void main(String[] args) {
		executeCommand(new SFI_DSS_D017(), args);
	}
	...
	protected boolean scrap(CrawlingProduct info) throws Exception {
		...
		WebElement renewalCycleEl = driver.findElement(By.xpath("//*[@class='renwScCycle']"));
		String renewalCycleText = renewalCycleEl.findElement(By.xpath("./dd")).getText().replace("자동갱신", "").trim();

		if (!renewalCycleText.equals(insTerm)) { throw new Exception(); }

		logger.info("환급유형 확인");
		String refundType = info.getProductKind();
		refundType = "만기환급형".equals(refundType) ? "일부환급형" : refundType;
		WebElement refundTypeEl = driver.findElement(By.xpath("//*[@class='refundCls']"));
		String refundTypeText = refundTypeEl.findElement(By.xpath("./dd")).getText().trim();

		if (!refundTypeText.equals(refundType)) { throw new Exception(); }

		logger.info("납입방법 확인");
		String napCycle = getNapCycleName(info.getNapCycle());
		WebElement napCycleEl = driver.findElement(By.xpath("//*[@class='paymentPeriod']"));
		String napCycleText = napCycleEl.findElement(By.xpath("./dd")).getText().trim();

		if (!napCycleText.equals(napCycle)) { throw new Exception(); }

		logger.info("플랜 설정");
		setPlan2(info.planSubName);
	...
	public void setTreaties(List<CrawlingTreaty> welgramTreatyList) throws SetTreatyException {
		...
		try {
			// 플랜 영역 thead
			WebElement $planAreaThead = helper.waitVisibilityOf(driver.findElement(By.id("plan-code")));
			...
			WebElement $treatyPopupDiv = helper.waitVisibilityOf(driver.findElement(By.xpath("//div[@class='modal-content']")));
			...
			script = "$(arguments[0]).find('span.pop-ref-txt').remove();";
			helper.executeJavascript(script, $treatyAreaTbody);

			// 원수사 특약명 수집
			for (WebElement $treatyTr : $targetTreatyTrList) {
				if (!helper.existElement($treatyTr, By.xpath("./th/p"))) {
					continue;
				}
			...
			for (String treatyName : dismatchedTreatyNameList) {
				...
				if (isToggleJoin) {
					...
					$treatyJoinInput = $treatyJoinLabel.findElement(By.xpath(".//input"));
			...
			// 특약에 대해 가입/미가입 처리를 진행하면 tr의 상태가 바꼈기 때문에 다시한번 element를 찾아줘야한다. 안그러면 StaleElementReference 예외 발생
			$treatyAreaTbody = $treatyPopupDiv.findElement(By.id("coverage-list"));
			$targetTreatyTrList = $treatyAreaTbody.findElements(By.xpath("./tr"));
			for (WebElement $treatyTr : $targetTreatyTrList) {

				if (helper.existElement($treatyTr, By.xpath("./td"))) {
					...
					//특약 가입여부 확인
					By by = By.xpath("./span[@class='pop-ref-txt']");
