...
public class SFI_CHL_D013 extends CrawlingSFIDirect {

	public static void main(String[] args) {
		executeCommand(new SFI_CHL_D013(), args);
	}
	...
	public void setTreaties(List<CrawlingTreaty> welgramTreatyList) throws SetTreatyException {
		...
		try {
			...
			List<String> strangeTreatyNameList = new ArrayList<>();                               //이상이 있는 특약명 리스트
			...
			for(String treatyName : dismatchedTreatyNameList) {
				...
				if(isToggleJoin) {
					...
					if($treatyJoinInput.isSelected()) {
						...
						//원수사에서 default로 "가입" 처리되어있다는 건 이상함. 우리쪽 가입설계에 해당 특약을 추가해야할 수도 있는 상황.
			...
			for(String treatyName : matchedTreatyNameList) {
				...
				if(isToggleJoin) {
					...
					if(!$treatyJoinInput.isSelected()) {
						...
						//원수사에서 default로 "미가입" 처리되어있다는 건 이상함. 우리쪽 가입설계에 해당 특약을 제거해야할 수도 있는 상황.
						strangeTreatyNameList.add(treatyName);
			...
			logger.info("=========================== 이상한 특약명 목록 ===========================");
			strangeTreatyNameList.forEach(t -> logger.info(t));
