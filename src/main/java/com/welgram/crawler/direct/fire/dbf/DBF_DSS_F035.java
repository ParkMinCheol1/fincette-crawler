...
public class DBF_DSS_F035 extends CrawlingDBFAnnounce {

    // 나에게맞춘플러스종합보험 2304 02종_자유설계_해약환급금지급형_납면적용(갱신보험기간)_갱신형
    public static void main(String[] args) {
        executeCommand(new DBF_DSS_F035(), args);
    }
    ...
    private void crawlFromHomepage(CrawlingProduct info) throws Exception {
        ...
        /*
            특이사항(바로 2종 선택하면 진행 불가)
            계산페이지 들어와서 바로 2종 선택 후 보험료계산은 안되고
            1종에서 보장목록확인 누르고 2종으로 넘어오면 계산 가능
         */
        ...
        logger.info("가입 유형 선택: 계산을 위한 임시선택");
        setProductType("sl_pan_cd", "01종_자유설계_해약환급금지급형_납면적용_세만기");

        logger.info("운전형태 : 자가용 고정");
        setVehicle("DRIVE_TYPE_CD", "자가용");

        logger.info("보험기간 선택: 계산을 위한 임시선택");
        WebElement $termSelect = driver.findElement(By.cssSelector("select[name=exp_pytr]"));
        setTerm($termSelect, "100세만기20년납");
        ...
        logger.info("가입 유형 선택: 다시 해당 상품의 유형으로 변경");
        setProductType("sl_pan_cd", info.planSubName);

        logger.info("계약 갱신 주기 선택");
        $termSelect = driver.findElement(By.cssSelector("select[name=pan_rnw_tpcd]"));
