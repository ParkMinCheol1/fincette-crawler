...
// 2023.04.03           | 최우진               | 대면 _저축보험
// TYL_SAV_F002         | 무배당수호천사더블테크보험Ⅲ
public class TYL_SAV_F002 extends CrawlingTYL {

    public static void main(String[] args) { executeCommand(new TYL_SAV_F002(), args); }
    ...
    protected boolean scrap(CrawlingProduct info) throws Exception {
        ...
        logger.info("START :: 무배당수호천사더블테크보험Ⅲ");

        logger.info("공시실에서 상품찾기");
        findInsuFromAnnounce("무배당수호천사더블테크보험Ⅲ");
        ...
        logger.info("주상품 설정");
        setPlanType("(무)수호천사 더블테크Ⅲ (기본형_P)");
        ...
        WebElement mainTreatyAssureMoneyEl = mainTreatyTr.findElement(By.xpath("./ancestor::tr//input[@title[contains(., '보험료')]]"));

        //3. 주계약의 가입금액 inputBox를 클릭한다.
        mainTreatyAssureMoneyEl.click();
        mainTreatyAssureMoneyEl.sendKeys(Keys.chord(Keys.CONTROL, "a", Keys.DELETE));
        mainTreatyAssureMoneyEl.sendKeys(String.valueOf(mainTreaty.assureMoney));
        ...
        getAnnounceShortReturnPremiumsNew(info);
    ...
    private void getAnnounceShortReturnPremiumsNew(CrawlingProduct info) {
        int unit = 1;
        ...
        for (WebElement tr : trList) {
            ...
            logger.info("|--일반해약환급률: {}", returnRate);
