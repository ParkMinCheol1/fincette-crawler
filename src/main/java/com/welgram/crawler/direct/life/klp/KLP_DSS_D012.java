...
public class KLP_DSS_D012 extends CrawlingKLP {



    public static void main(String[] args) {
        executeCommand(new KLP_DSS_D012(), args);
    }
    ...
    private void crawlFromHomepage(CrawlingProduct info) throws Exception {
            ...
            */
            ...
            logger.info("진단보험금 입력");
            setAssureMoneySelect(info);
    ...
    protected void setAssureMoneySelect(CrawlingProduct info) throws Exception{
        ...
        for(int i=0; i<elements.size(); i++){
            ...
            for(int j=0; j<info.treatyList.size(); j++){
                ...
                logger.info("상품마스터 금액 : "+info.treatyList.get(j).assureMoney);
                logger.info("페이지 금액 : "+elements.get(i).findElement(By.cssSelector("input")).getAttribute("value").trim());

                assureMoney = Integer.toString(info.treatyList.get(j).assureMoney);

                if(Integer.toString(info.treatyList.get(j).assureMoney).equals(elements.get(i).findElement(By.cssSelector("input")).getAttribute("value").trim())){
        ...
        if(!result){
            throw new Exception("현재 상품마스터의 진단보험금 : "+assureMoney+"원이 존재하지 않아 클릭할 수 없습니다.");
        }
