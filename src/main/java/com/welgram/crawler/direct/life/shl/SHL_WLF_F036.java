...
// SHL_WLF_F036         | 신한멋진종신보험v2.0(무배당, 해약환급금 일부지급형) 라이트형
public class SHL_WLF_F036 extends CrawlingSHL {

    public static void main(String[] args) { executeCommand(new SHL_WLF_F036(), args); }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {

        logger.info("START :: 신한멋진종신보험v2.0(무배당, 해약환급금 일부지급형) 라이트형");
        logger.info("대면보험의 경우 공시실의 정보를 크롤링합니다. [ SHL_WLF_'F'### ]");
        WaitUtil.loading(2);

        try {
            logger.info("검색창에서 상품명 : [ {} ]을 조회합니다", "신한멋진종신보험v2.0");
            driver.findElement(By.id("meta04")).sendKeys("신한멋진종신보험v2.0");
            WaitUtil.waitFor(2);
        ...
        try {
            ...
            selctInsForm.selectByVisibleText("해약환급금일부지급형");               // SHL_WLF_F036의 대표가설은 해약환급금일부지급형 입니다
            WaitUtil.loading(2);
        ...
        try{
            ...
            selctInsKind.selectByVisibleText("라이트형");                              // 간편심사형
            WaitUtil.waitFor(2);
        } catch(Exception e) {
