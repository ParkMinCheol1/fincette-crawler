...
public class LIN_WLF_F014 extends CrawlingLINAnnounce { // 무배당THE건강해지는종신보험(해약환급금미지급형)_가입금액형

    public static void main(String[] args) {
        executeCommand(new LIN_WLF_F014(), args);
    }
    ...
    private void disClosureRoomCrawling(CrawlingProduct info) throws Exception {
        ...
        String genderOpt = (info.getGender() == MALE) ? "gender1_M" : "gender1_F";
        String genderText = (info.getGender() == MALE) ? "남자" : "여자";

        logger.info("LIN_WLF_F014 :: {}", info.getProductName());
        WaitUtil.waitFor(1);
