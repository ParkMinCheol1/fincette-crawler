...
// 2023.04.26 | 최우진 | 교보실속간편가입건강종신보험(무배당,보증비용부과형)
public class KYO_WLF_F024 extends CrawlingKYO {

	public static void main(String[] args) {
		executeCommand(new KYO_WLF_F024(), args);
	}
	...
	private void doCrawlInsurancePublic(CrawlingProduct info) throws Exception {
		...
		//helper.doClick(driver.findElement(By.cssSelector("#contents > div.ut-tabs.h > ul > li:nth-child(1) > a")));
		logger.info("공시실 진입 후 종신/정기 버튼 클릭");
		element = driver.findElement(By.linkText("종신/정기"));
		...
		for (int i=0; i<elements.size(); i++) {
			...
			if (siteProductName.trim().equals("교보실속간편가입건강종신보험(무배당,보증비용부과형)")) {			// 이름주의
...
//				wait.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("[class~='ui-loading']")));
				...
				if (item.insTerm.equals("종신보장")) { item.insTerm = "종신"; }
				...
				// 보기 납기가 같은경우 납입기간 처리
				if (item.insTerm.equals(item.napTerm)) { item.napTerm = "전기납"; }
				...
				element.sendKeys(Keys.DELETE);
				element.sendKeys(Keys.DELETE);
				element.sendKeys(item.assureMoney / 1_0000 + "");
			}
		}
