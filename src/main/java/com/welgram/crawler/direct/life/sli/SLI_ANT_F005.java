...
public class SLI_ANT_F005 extends CrawlingSLIAnnounce {

	public static void main(String[] args) {
		executeCommand(new SLI_ANT_F005(), args);
	}
	...
	protected boolean scrap(CrawlingProduct info) throws Exception {
		...
		logger.info("고액계약 설정");
		setLargeAmountContract(info.getTextType().split(",")[1].trim());
		...
		setInsTerm(info.getAnnuityAge(), By.id("insrPrdTypValCd_A014801A03"));

		logger.info("주계약 납입기간 설정");
		setNapTerm(info.getNapTerm(), By.id("padPrd_A014801A03"));

		logger.info("주계약 보험료(가입금액) 설정");
		setAssureMoney(info.getAssureMoney(), By.id("price_A014801A03"));
	...
	public void crawlAnnuityPremium(Object... obj) throws CommonCrawlerException {
		...
		try{
			...
			String[] counts = {"10회", "20회", "30회", "100회"};
			...
			String whl10Y = whlMap.getOrDefault("10회", "0");
			String whl20Y = whlMap.getOrDefault("20회", "0");
			String whl30Y = whlMap.getOrDefault("30회", "0");
			String whl100A = whlMap.getOrDefault("100회", "0");
			...
			WebElement $fxdTh = driver.findElement(By.xpath("//th[normalize-space()='확정기간연금플러스형']"));
