...
// SHL_WLF_F013         | 신한진심을품은종신보험S(무배당, 해약환급금 일부지급형) 기본형(미부가형)
public class SHL_WLF_F013 extends CrawlingSHL {



    public static void main(String[] args) { executeCommand(new SHL_WLF_F013(), args); }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {
        logger.info("START :: SHL_WLF_F013 :: 신한진심을품은종신보험S(무배당, 해약환급금 일부지급형) 기본형(미부가형)");
        ...
        for(int i = 0; i < arrTType.length; i++) {
            ...
            // 0 : 신한진심을품은종신보험S(무배당, 해약환급금 일부지급형)
            // 1 : 기본형(미부가형)
            // 2 :
        }
        WaitUtil.loading(2);
