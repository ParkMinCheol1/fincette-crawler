...
// 2023.03.22           | 최우진           | 대면 종신보험
// SHL_WLF_F028         | 로지종신보험(무배당, 해약환급금 일부지급형)
public class SHL_WLF_F028 extends CrawlingSHL {

    public static void main(String[] args) {
        executeCommand(new SHL_WLF_F028(), args);
    }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {
        logger.info("START :: 로지종신보험(무배당, 해약환급금 일부지급형)");
        ...
        for(int i = 0; i < arrTType.length; i++) {
            ...
            // 0 : 로지종신보험(무배당, 해약환급금 일부지급형)
            // 1 : 체증형
            // 2 : 1종 보증형
        }
        WaitUtil.loading(2);

        logger.info("검색창에서 상품명 : [ {} ]을 조회합니다", "로지종신보험(무배당, 해약환급금 일부지급형)");
        driver.findElement(By.id("meta04")).sendKeys("로지종신보험(무배당, 해약환급금 일부지급형)");
        WaitUtil.waitFor(2);
...
//            if(info.getNapTerm().contains("10년")) {
//                selctInsKind.selectByVisibleText("10년납이하");
//            } else {
//                selctInsKind.selectByVisibleText("10년납초과");
//            }
            WaitUtil.loading(2);
        } catch(Exception e) {
