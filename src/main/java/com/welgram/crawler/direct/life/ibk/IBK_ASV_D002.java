...
public class IBK_ASV_D002 extends CrawlingIBK {

    public static void main(String[] args) {
        executeCommand(new IBK_ASV_D002(), args);
    }
    ...
    private int userSite(CrawlingProduct info) {
        ...
        try {
            ...
            driver.findElement(By.id("txtPayMoney02"))
            ...
            // 납입기간
            {
                ...
                dropdown.findElement(
                    By.xpath("./option[. = '" + info.napTerm.replaceAll("년", "") + "']")).click();
...
//
    ...
    protected void getPension(CrawlingProduct info) throws Exception {
        ...
        String whl20yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(2) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");      //종신 20년 연금수령액
        String whl30yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(3) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");      //종신 30년 연금수령액
        String whl100aPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(4) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");    //종신 100세 연금수령액
        String fxd10yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(6) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");     //확정 10년 연금수령액
        String fxd15yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(7) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");     //확정 15년 연금수령액
        String fxd20yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(8) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");     //확정 20년 연금수령액
        String fxd25yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(9) > td:nth-child(4)")).getText()
            .replaceAll("[^0-9]", "");     //확정 25년 연금수령액
        String fxd30yPremium = driver.findElement(
                By.cssSelector("#b1 > table > tbody > tr:nth-child(10) > td:nth-child(4)")).getText()
        ...
        if ("종신 10년".equals(annuityType)) {
            ...
            fixedAnnuityPremium = fxd10yPremium;
        } else if ("확정 15년".equals(annuityType)) {
            fixedAnnuityPremium = fxd15yPremium;
        } else if ("확정 20년".equals(annuityType)) {
            fixedAnnuityPremium = fxd20yPremium;
        } else if ("확정 25년".equals(annuityType)) {
            fixedAnnuityPremium = fxd25yPremium;
        } else if ("확정 30년".equals(annuityType)) {
            fixedAnnuityPremium = fxd30yPremium;
        }
    ...
    private boolean publicSite(CrawlingProduct info) {
        ...
        try {
            ...
            logger.info("보험료할인 여부: 적립가산형 선택(50만원초과)");
            setDiscount(500000, Integer.valueOf(info.assureMoney));
