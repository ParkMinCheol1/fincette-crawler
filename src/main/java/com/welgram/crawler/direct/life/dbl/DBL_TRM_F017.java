...
public class DBL_TRM_F017 extends CrawlingDBLAnnounce {

    public static void main(String[] args) {

        executeCommand(new DBL_TRM_F017(), args);
    }
    ...
    public void crawlReturnMoneyListAll(Object... obj) throws ReturnMoneyListCrawlerException {
        ...
        try {
            ...
            for (WebElement tr : elements) {
                ...
                String term = "";
                int premiumSum = 0;
                int returnMoneyMin = 0;
                String returnRateMin = "";
                int returnMoneyAvg = 0;
                ...
                premiumSum = Integer.parseInt(tr.findElements(By.tagName("td")).get(5).getText().replaceAll("\\D", "")) * unit;

                returnMoneyMin = Integer.parseInt(tr.findElements(By.tagName("td")).get(6).getText().replaceAll("\\D", "")) * unit;
                returnRateMin = tr.findElements(By.tagName("td")).get(7).getText();

                returnMoneyAvg = Integer.parseInt(tr.findElements(By.tagName("td")).get(8).getText().replaceAll("\\D", "")) * unit;
                returnRateAvg = tr.findElements(By.tagName("td")).get(9).getText();

                returnMoney = Integer.parseInt(tr.findElements(By.tagName("td")).get(10).getText().replaceAll("\\D", "")) * unit;
