...
// SHL_WLF_F029         | 신한간편가입용감한종신보험(무배당, 해약환급금 일부지급형)
public class SHL_WLF_F029 extends CrawlingSHL {

    public static void main(String[] args) {
        executeCommand(new SHL_WLF_F029(), args);
    }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {

        logger.info("START :: 신한간편가입용감한종신보험(무배당, 해약환급금 일부지급형)");
        logger.info("대면보험의 경우 공시실의 정보를 크롤링합니다. [ SHL_WLF_'F'### ]");
        WaitUtil.loading(2);

        try {
            logger.info("검색창에서 상품명 : [ {} ]을 조회합니다", "신한간편가입용감한종신보험(무배당, 해약환급금 일부지급형)");
            driver.findElement(By.id("meta04")).sendKeys("신한간편가입용감한종신보험(무배당, 해약환급금 일부지급형)");
            WaitUtil.waitFor(2);
        ...
        try {
            ...
            selctInsForm.selectByVisibleText("일반형");               // SHL_WLF_F030의 대표가설은 일반심사형입니다
            WaitUtil.loading(2);
