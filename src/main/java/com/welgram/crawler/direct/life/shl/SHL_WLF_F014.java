...
// SHL_WLF_F0014        | 신한탄탄든든유니버설종신보험(무배당) 평준형
public class SHL_WLF_F014 extends CrawlingSHL {

    public static void main(String[] args) {
        executeCommand(new SHL_WLF_F014(), args);
    }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {
        logger.info("START :: 신한탄탄든든유니버설종신보험(무배당) 평준형");
...
//        helper.doSendKeys(By.xpath("//input[@type='text'][@title='생년월일']"), info.getFullBirth());
        ...
        try {
            Select selctInsForm = new Select(driver.findElement(By.xpath("//select[@title='보험종류']")));
