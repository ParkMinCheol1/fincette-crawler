 ...
 */
// 2022.07.12       | 최우진           | 다이렉트_암보험
// SHL_CCR_D009     | 신한인터넷암보험(무배당,해약환급금 미지급형)
public class SHL_CCR_D009 extends CrawlingSHL {



    public static void main(String[] args) {
        executeCommand(new SHL_CCR_D009(), args);
    }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {
        logger.info("START :: SHL_CCR_D009 :: 신한인터넷암보험(무배당,해약환급금 미지급형)");
        WaitUtil.loading(4);
        ...
        logger.info("'신한인터넷암보험(무배당, 일반형) 선택");
        ...
        try {
            ...
            throw new CommonCrawlerException("보장기간 설정이 잘못되었습니다");
        }
        ...
        try {
            ...
            throw new CommonCrawlerException("보험가입금액 설정이 잘못되었습니다");
        }

        logger.info("암사망특약(무배당) 가입금액 ");
        boolean isTreaty = false;
        for (int i = 0; i < info.getTreatyList().size(); i++) {
            String treatyName = info.getTreatyList().get(i).treatyName;
            int treatyAmt = info.getTreatyList().get(i).assureMoney;
            logger.info("특약리스트 " + i + " : " + treatyName);
            logger.info("특약리스트 " + i + " : " + treatyAmt);
            if (treatyName.contains("암사망보험금")) {
                ...
                logger.info("암사망보험금 : " + treatyMoney + "원");
                isTreaty = true;
            }
        }
        if (!isTreaty) {
            helper.click(By.xpath("//span[text()='포함']//ancestor::label[@for='switch1']"));            // 미포함 처리 (포함 >> 미포함)
        }
...
//        setInsuranceKind("해약환급금미지급형"); // 보험종류
//        setNapCycle(info.getNapCycle()); // 납입주기
//
