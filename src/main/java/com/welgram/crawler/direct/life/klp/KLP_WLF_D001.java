...
public class KLP_WLF_D001 extends CrawlingKLP {



	public static void main(String[] args) {
		executeCommand(new KLP_WLF_D001(), args);
	}
		...
		if (value1 != 0) {
			...
			for (WebElement li : elements) {
				if ((value2 + "천만원").equals(li.findElement(By.tagName("span")).getText().trim())) {
					...
					//천만원이 선택되어야 하는 상황에 억단위가 기본 단위로 선택되면 경고창이 2번뜸. 친만원대 값을 적용하려면 필요함.
					if (isAlertShowed()) {
						...
						driver.findElement(By.cssSelector("#frmSelfInfo > ul > li:nth-child(1) > div.box_middle.type_7 > ul > li:nth-child(1) > span")).click();
						WaitUtil.waitFor(1);
