...
/**
 * @author 무)KB,알기쉬운종신보험(해약환급금 과소지급형) 베타추구형 : 해약환급률 90%이상
 */
public class KBL_WLF_D002 extends CrawlingKBL {



    public static void main(String[] args) {
        executeCommand(new KBL_WLF_D002(), args);
    }
...
//            mobileCrawling(info);
    ...
    protected void webCrawling(CrawlingProduct info) throws Exception{
        ...
        logger.info("알파추구형 선택");
        driver.findElement(By.xpath("//*[span[contains(., '알파추구형')]]")).click();

        try {
            logger.info("고혈압 당뇨 [아니오] 선택");
            driver
                .findElement(By.xpath("//*[@id='modalProductType_quest5_2']//*[text()='아니오']"))
                .click();
        } catch (Exception var12) {
            logger.info("고혈압, 당뇨 질문 없음");
        }

        logger.info("[확인] 버튼 클릭");
        driver.findElement(By.id("buttonCalculationForOnlinProductType")).click();
...
//        logger.info("연금전환나이 :: {}", annuityChangeAge + "세");
//        selectDropDown_front(By.id("annuityTansWrap"), annuityChangeAge);
//
//        logger.info("연금개시나이 :: {}", info.annAge + "세");
//        selectDropDown_front(By.id("annuityStartAgeWrap"), info.annAge);
        ...
        lifePlanReturnMoney(info);

//        logger.info("연금수령액 스크랩");
//        lifeAnnuityPremium(info);
    ...
    protected void mobileCrawling(CrawlingProduct info) throws Exception{
        ...
        helper.click(driver.findElement(By.id("entry")));

        logger.info("생년월일 :: {}", info.fullBirth);
        WaitUtil.waitFor(5);
        driver.switchTo().frame(0);
        helper.sendKeys3_check(By.id("custBirth"), info.fullBirth);
        ...
        logger.info("연금개시 나이 :: {} 세", info.annuityAge);
        setSelectBox("#pensStAge", info.annuityAge, iframe);
