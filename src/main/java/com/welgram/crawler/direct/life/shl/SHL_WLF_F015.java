...
// SHL_WLF_F015         | 신한탄탄든든유니버설종신보험(무배당) 체증형
public class SHL_WLF_F015 extends CrawlingSHL {

    public static void main(String[] args) {
        executeCommand(new SHL_WLF_F015(), args);
    }

    @Override
    protected boolean scrap(CrawlingProduct info) throws Exception {
        logger.info("START :: 신한탄탄든든유니버설종신보험(무배당) 체증형");
...
//			driver.findElement(By.xpath("//*[@id='popu_100000000000500']/div[2]/div/div/div[3]/button")).click();
        ...
        logger.info("검색창에서 상품명 : [ {} ]을 조회합니다", "신한탄탄든든유니버설종신보험(무배당)");
        driver.findElement(By.id("meta04")).sendKeys("신한탄탄든든유니버설종신보험(무배당)");
//        helper.doSendKeys(By.id("meta04"), "신한탄탄든든유니버설종신보험(무배당)");
        WaitUtil.waitFor(2);
...
//        wait.until(ExpectedConditions.invisibilityOfAllElements(driver.findElements(By.xpath("//div[@class[contains(., 'load')]]"))));
        ...
        try {
            By by = By.xpath("//select[@title='보험종류']");
//            String pType = String.valueOf(info.getPlanType());
            wait.until(ExpectedConditions.elementToBeClickable(by));
            Select selctInsKind = new Select(driver.findElement(by));
            selctInsKind.selectByVisibleText(arrTType[1]);

            wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[@class='loading']")));
            String selResult = selctInsKind.getAllSelectedOptions().get(0).getText();

            // todo | 재귀호출로 바꿀 수 없나
            for(int cnt = 0; cnt < 3; cnt++) {
                selctInsKind.selectByVisibleText(arrTType[1]);
                if(arrTType[1].equals(selResult)) {
                    break;
                }
            }

            if(arrTType[1].equals(selResult)) {
                logger.info("  텍스트타입 eq 셀렉트박스");
                logger.info("  ERROR TEXTTYPE  ::: " + arrTType[1]);
                logger.info("  ERROR SELRESULT ::: " + selResult);

            } else {
                logger.info("  텍스트타입 != 셀렉트박스");
                logger.info("  ERROR TEXTTYPE  ::: " + arrTType[1]);
                logger.info("  ERROR SELRESULT ::: " + selResult);
                // todo | 보험형태설정 에러... wait.until이 잘안먹나...
                throw new CommonCrawlerException("잘못된 보험형태 설정입니다");
            }
            logger.info(selctInsKind.getAllSelectedOptions().get(0).getText());

            WaitUtil.loading(2);
        } catch (Exception e) {
            throw new CommonCrawlerException("보험형태를 설정할 수 없습니다" + e.getMessage());
        }
        ...
        try{
            ...
            selectInsPeriod.selectByVisibleText(arrTType[0]);
//            selectInsPeriod.selectByVisibleText("종신");
//            selectInsPeriod.selectByVisibleText("종신(55세형)");
